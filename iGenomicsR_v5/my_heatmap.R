...
fun_order_Genes <- function(genes, order_by, selected_samples, DB){
  t <- try(
...
# mutation based 
########################################################################
my_heatmap_mutation <- function(DB, mutation_genes, rna_genes, protein_genes, clinical_lab,
  ...
  sample_order_res <- fun_order_samples(mutation_genes, rna_genes, protein_genes, clinical_lab, order_by, selected_samples, clust_para,order_clin_feature, image_features, DB)
  ordered_samples <- sample_order_res[["ordered_samples"]] 
  
  if(length(mutation_genes) > 1 & sort.mutation){
    mutation_genes <- fun_order_Genes(mutation_genes, "Mutation_gene", ordered_samples, DB)
  }
  if(length(rna_genes) > 2 & sort.rna){
    rna_genes <- fun_order_Genes(rna_genes, "RNA", ordered_samples, DB)
  }
  if(length(image_features) > 2 & sort.image){
    image_features <- fun_order_Genes(image_features, "Image", ordered_samples, DB)
  }
  if(length(protein_genes) > 2 & sort.protein){
    protein_genes <- fun_order_Genes(protein_genes, "Protein", ordered_samples, DB)
  }
...
my_heatmap_rna <- function(DB, mode, clust_para=list(method="hc"), mutation_genes, image_features = vector(),
...
# protein based
########################################################################
my_heatmap_protein <- function(DB, mode, mutation_genes, image_features = vector(), rna_genes,
  ...
  # order by user specified genes
  if(mode==0){
    my_heatmap_mutation(DB, mutation_genes=mutation_genes, image_features=image_features, protein_genes=protein_genes, 
